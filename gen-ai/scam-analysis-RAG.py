...
class standard_output(BaseModel):
    description: str = Field(description = "description of the message provided from users")
    likelihood_of_scam: str = Field(description = "how likely is the message to be a Scam, e.g. not likely a scam, likely a scam, highly likely a scam")
    score: str = Field(description = "generated AI score between 0 to 100, with 100 being highly suspected Scam and 0 being not likely to be a Scam. ")
    explanation: str = Field(description = "short explanation of why the score was given, any characteristics of scam messages")
    type_of_scam: str = Field(description = "Based on the knowledge, categorise the scam message based on the list of category")
    suggestions: str = Field(description = "short description of any suggestions to combat against scam messages.")

def scam_analysis_RAG(input_message:str):
    llm = ChatOpenAI(model_name = "gpt-3.5-turbo", temperature=0.1)
    embedding_function = OpenAIEmbeddings()
    # Load from disk
    vectorstores = Chroma(persist_directory="./chroma_db",embedding_function = embedding_function)
    ...
    rag_chain = {"context": retriever, "description": RunnablePassthrough()} | prompt | llm | parser
